...
int main() {
    try {
        ...
        std::cout << "=== tinybert量化模型推理程序（CSV测试版）===" << std::endl;
        ...
        while (true) {
            ...
            if (choice == "1") {
                ...
                double accuracy = csv_data.empty() ? 0.0 :
                    static_cast<double>(correct_predictions) / csv_data.size() * 100.0;
                ...
                std::cout << "当前总内存占用: " << getCurrentMemoryUsage() << " KB" << std::endl;
                std::cout << "正确预测数量: " << correct_predictions << "/" << csv_data.size() << std::endl;
                std::cout << "准确率: " << std::fixed << std::setprecision(2) << accuracy << "%" << std::endl;
                std::cout << "------------------------------------" << std::endl;
                
                // 保存详细结果到CSV文件
                std::string result_file = "tiny_int8_test_results.csv";
                ...
                if (out_file.is_open()) {
                    ...
                    for (size_t i = 0; i < csv_data.size(); ++i) {
                        ...
                        double inference_time = 0.0;
                        size_t memory_usage = 0;
                        int predicted_class = performInference(session, tokenizer, row.text, inference_time, memory_usage);
                        bool is_correct = (predicted_class == row.label);
