...
int performInference(Ort::Session& session, BertTokenizer& tokenizer, const std::string& text, 
                    double& inference_time, size_t& memory_usage, bool verbose = false) {
    ...
    try {
        memory_usage = getMemoryDelta([&]() {
            // 对文本进行编码
            auto [input_ids, attention_mask, token_type_ids] = tokenizer.encode(text, 128, true);  // 使用固定长度填充，与Python版本一致
            ...
            const char* output_names[] = {"logits"};  // 确保使用正确的输出节点名
...
int main() {
    try {
        ...
        std::cout << "=== BERT标准模型推理程序（CSV测试版）===" << std::endl;
        
        // 检查模型文件
        std::string model_path = "model/model.onnx";
        if (!std::filesystem::exists(model_path)) {
            std::cerr << "错误: 未找到标准模型文件! 尝试使用其他路径..." << std::endl;
            
            // 尝试其他可能的路径
            model_path = "../model/model.onnx";
            if (!std::filesystem::exists(model_path)) {
                model_path = "../../cpp/model/model.onnx";
                if (!std::filesystem::exists(model_path)) {
                    std::cerr << "错误: 无法找到标准模型文件!" << std::endl;
                    return 1;
                }
            }
        }
        ...
        // 初始化ONNX Runtime
        Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "bert-standard-inference");
        ...
        while (true) {
            ...
            if (choice == "1") {
                ...
                try {
                    // 启用详细输出
                    int predicted_class = performInference(session, tokenizer, text, inference_time, memory_usage, true);
                ...
                // 保存详细结果到CSV文件
                std::string result_file = "bert_standard_test_results.csv";
        ...
        std::cout << "BERT标准模型推理程序已退出" << std::endl;
        return 0;
