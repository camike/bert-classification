...
// 标准模型推理结果结构体
struct StandardInferenceResult {
    int predicted_class;
    float max_score;
...
// 执行标准模型推理
StandardInferenceResult runStandardInference(Ort::Session& session, BertTokenizer& tokenizer, const std::string& text) {
    // 记录初始内存
...
int main() {
    try {
        ...
        std::cout << "=== BERT标准模型推理程序 ===" << std::endl;
        
        // 检查模型文件
        std::string model_path = "model/model.onnx";
        if (!std::filesystem::exists(model_path)) {
            std::cerr << "错误: 未找到标准模型文件!" << std::endl;
            return 1;
        }
        ...
        // 加载标准模型
            ...
            std::wstring widestr = converter.from_bytes(model_path);
        ...
        while (true) {
            ...
            // 执行标准模型推理
            auto result = runStandardInference(session, tokenizer, text);
