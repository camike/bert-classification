...
int main() {
    try {
        ...
        std::cout << "=== BERT INT8量化模型推理程序（CSV测试版）===" << std::endl;
        
        // 检查模型文件
        std::string model_path = "model/bert_model_quant.onnx";
        if (!std::filesystem::exists(model_path)) {
            std::cerr << "错误: 未找到INT8量化模型文件! 尝试使用其他路径..." << std::endl;
            
            // 尝试其他可能的路径
            model_path = "../model/bert_model_quant.onnx";
            if (!std::filesystem::exists(model_path)) {
                model_path = "../../cpp/model/bert_model_quant.onnx";
                if (!std::filesystem::exists(model_path)) {
                    std::cerr << "错误: 无法找到INT8量化模型文件!" << std::endl;
                    return 1;
                }
            }
        }
        ...
        while (true) {
            ...
            if (choice == "1") {
                ...
                // 保存详细结果到CSV文件
                std::string result_file = "bert_int8_test_results.csv";
        ...
        std::cout << "BERT INT8量化模型推理程序已退出" << std::endl;
        return 0;
