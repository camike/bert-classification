...
int performInference(Ort::Session& session, BertTokenizer& tokenizer, const std::string& text, 
    ...
    try {
        memory_usage = getMemoryDelta([&]() {
            ...
            const char* output_names[] = {"logits"};  // 输出节点名称，lmb_unquant.onnx使用logits
...
int main() {
    try {
        ...
        std::cout << "=== LMB未量化模型推理程序（CSV测试版）===" << std::endl;
        
        // 检查模型文件
        std::string model_path = "model/lmb_unquant.onnx";
        if (!std::filesystem::exists(model_path)) {
            std::cerr << "错误: 未找到LMB未量化模型文件! 尝试使用其他路径..." << std::endl;
            
            // 尝试其他可能的路径
            model_path = "../model/lmb_unquant.onnx";
            if (!std::filesystem::exists(model_path)) {
                model_path = "../../cpp/model/lmb_unquant.onnx";
                if (!std::filesystem::exists(model_path)) {
                    std::cerr << "错误: 无法找到LMB未量化模型文件!" << std::endl;
                    return 1;
                }
            }
        }
        ...
        // 初始化ONNX Runtime
        Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "lmb-unquant-inference");
        ...
        while (true) {
            ...
            if (choice == "1") {
                ...
                // 保存详细结果到CSV文件
                std::string result_file = "lmb_unquant_test_results.csv";
        ...
        std::cout << "LMB未量化模型推理程序已退出" << std::endl;
        return 0;
