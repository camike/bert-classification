            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration", ascii=True)):
                ...
                if (global_step + 1) % args.eval_step == 0:
                    ...
                    if save_model:
                        ...
                        try:
                            # 创建ONNX输出路径
                            onnx_output_path = os.path.join(args.output_dir, "model.onnx")

                            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                            
                            onnx_model_to_save = student_model
                            # 确保模型在评估模式
                            onnx_model_to_save.eval()
                            
                            # 创建示例输入（根据模型的最大序列长度）
                            batch_size = 1  # ONNX导出通常使用batch_size=1
                            seq_length = args.max_seq_length
                            
                            # 准备虚拟输入（与模型实际输入格式匹配）
                            dummy_input = {
                                "input_ids": torch.randint(0, 21100, (batch_size, seq_length)).long().to(device),
                                "attention_mask": torch.ones((batch_size, seq_length)).long().to(device),
                            }
                            
                            # 如果模型使用token_type_ids（如BERT）
                            if "token_type_ids" in onnx_model_to_save.forward.__code__.co_varnames:
                                dummy_input["token_type_ids"] = torch.zeros((batch_size, seq_length)).long().to(device)
                            
                            # 动态轴配置（使模型支持可变batch_size和sequence_length）
                            dynamic_axes = {
                            ...
                            torch.onnx.export(
                                ...
                                opset_version=17,  # 推荐使用opset 14或更高
