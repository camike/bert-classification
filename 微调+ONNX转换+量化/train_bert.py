...
# 1. 标签映射（1处需要修改）: 第105行
# 2. test trainer文件保存路径（1处需要修改）: 第156行
# 3. 微调模型文件保存路径（1处需要修改）: 第192行
# 4. 训练集路径（1处需要修改）: 第89行
################################################
...
# 设置cuBLAS确定性环境变量
# 这是根据错误信息需要设置的环境变量，用于确保cuBLAS操作的确定性
...
# 尝试使用确定性算法（如果PyTorch版本支持）
try:
    # 在设置CUBLAS_WORKSPACE_CONFIG后使用确定性算法
...
# 创建固定的tokenizer
fixed_tokenizer_dir = "./fixed_tokenizer"
...
# 加载固定的tokenizer
tokenizer = AutoTokenizer.from_pretrained(fixed_tokenizer_dir)
...
def compute_metrics(eval_pred):
    ...
    try:
        ...
        # 使用torch.nn.functional.softmax而不是torch.softmax
...
# 创建输出目录
output_dir = "test_trainer_bert_full_data_128" # 修改为test trainer文件保存路径（3）
...
# 确保模型输出目录存在
model_output_dir = "trained_model_bert_full_data_128" # 修改为微调模型文件保存路径（4）
