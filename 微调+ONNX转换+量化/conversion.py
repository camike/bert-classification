######################################################
# 本脚本用于将PyTorch训练模型转换为ONNX格式
#
# 功能说明：
# - 输入: PyTorch训练模型
# - 输出: ONNX格式的模型
#
# 需要修改的配置项（共6处需要修改，已在代码中用注释"x"标记）：
# 1. 模型路径（4处需要修改）：第24行、第27行、第30行、第33行
# 2. 标签定义（1处需要修改）:第38行
# 3. 测试文本（1处需要修改）：第129行
######################################################
...
MODEL_DIR = os.path.expanduser("~/Desktop/quantization/trained_model_bert_full_data_128") # 需要改路径（1）

# ONNX输出目录 (确保此目录已存在)
ONNX_OUTPUT_DIR = os.path.expanduser("~/Desktop/quantization") # 需要改路径（2）

# ONNX文件名 (不需要带路径)
ONNX_FILENAME = "trained_model_bert_full_data_128.onnx" # 需要改路径（3）

# 测试文件路径 (可选)
TEST_FILE_PATH = os.path.expanduser("~/Desktop/quantization/test.txt") # 需要改路径（4）
# ================================================================
